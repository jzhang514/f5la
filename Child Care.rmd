...
hc<-hchart(internet, "column",
    ...
    hc_title(
    text = "Homes with Broadband Internet Access such as Cable, Fiber Optic or DSL in Best Start Geographies",
    margin = 20,
    align = "left",
...
hchart(internet, "column",
    ...
    hc_title(
    text = "Families who own One or More Technology Devices in Best Start Geographies",
    margin = 20,
    align = "left",
    style = list(useHTML = TRUE, fontWeight="bold")
    )%>%
...
### Families who own an Age 0 to 5 Child and a K-12 Aged Child
```{r}
...
#highcharts
 ...
 children_long %>%
   mutate(variable=ifelse(pct_measure %in% 'pct_hh_childund6', 'Percentage of Children Under 6 Years Old', 'Percentage of Children between 6 and 17 Years Old'))%>%
...
#get custom order of stacking bar graph
 prek_long <- within(prek_long, variable <- factor(variable, levels=c("Enrolled Children", "Eligible Children","Total Children")))
...
#get custom order of stacking bar graph
 it_long <- within(it_long, variable <- factor(variable, levels=c("Enrolled Children", "Eligible Children","Total Children")))
