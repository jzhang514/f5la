...
```
<br>Source: 2018 ACS 5-Year Estimates
...
#highcharts
 ...
 hc<-  hchart(race, "column",
         hcaes(x=best_start, y=pct_value, group=variable),
          stacking = "normal",
          tooltip = list(pointFormat = "{point.pct_measure}: {point.pct_value:,.1f}%<br>
                         {point.num_measure}: {point.num_value}"))%>%
...
leaflet(ct) %>%
  ...
  addPolygons(data=ct, color="#666666", fillColor=~pal(family_medianincome), weight = .85, smoothFactor = 0.5,
...
```
<br>Source: 2018 5-year American Census Survey (ACS) Estimates Table S1903
...
kable(regions, col.names=c(""))%>%
  ...
  pack_rows("Region 1", 1,4) %>%
  pack_rows("Region 2", 5, 8)%>%
  pack_rows("Region 3", 9, 10)%>%
  pack_rows("Region 4", 11, 12)%>%
  pack_rows("Region 5", 13, 14)
