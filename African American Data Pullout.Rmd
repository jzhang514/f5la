...
popup<-paste0(label = paste0("<span style='font-size: 14px; font-weight: bold'>", htmlEscape(ct$NAMELSAD), "</span></br>", "Black Alone or in Combination</br>Population Percent: ",ct$pct_black_aoic_pop,"%","</br></br>","Black Alone or in Combination</br>Population Total: ",ct$black_aoic_pop))
...
leaflet(height=585,width=785) %>%
 ...
 addLegend("bottomright", pal=pal, values = ct$pct_black_aoic_pop,
            title = "Percent Black Alone or in Combination Population")



```
Data Source: American Community Survey 2015-2019 Estimates. 
...
# Racism across systems further contributs to health disparites through ACEs and increases allostatic load
...
aces%>%
   ...
   hc_yAxis(title = list(text = ""), 
    ...
    hc_caption(
    text = paste0('<a href="https://www.cdph.ca.gov/Programs/CCDPHP/DCDIC/SACB/CDPH%20Document%20Library/Essentials%20for%20Childhood%20Initiative/ACEs-BRFSS-Update_final%2010.26.20.pdf">Data Source: BRFSS.</a>'
                ),
    align = "left",
    style=list(fontSize='12px')
  ) %>% 
...
Centuries of discrimination and racism in policies reduced the earnings and wealth accumulation abilities of Black households. <i>[The Color of Wealth](http://www.aasc.ucla.edu/besol/Color_of_Wealth_Report.pdf)</i> and PPIC research further talks about wealth gaps in the LA County and California context.
...
rcm%>%
   filter(variable=='Black' | variable=='Total')%>%
  mutate(variable=ifelse(variable %in% 'Black', 'Black Households', 'Total Households'))%>%
  mutate(geoid=paste0('Supervisorial District ',geoid))%>%
  
    hchart("bar",
         hcaes(x=geoid, y=pct_below_rcm, group=variable),
          tooltip = list(pointFormat = "Percent Below Real Cost Measure: {point.pct_below_rcm:.0f}%"))%>%
    ...
    hc_caption(
    text = paste0('<a href="https://www.unitedway.org/">Data Source: United Way.</a>'
                ),
    align = "left",
    style=list(fontSize='12px')
  ) %>% 
...
# Our housing system burdens Black families the most and provide opportunities for the least housing
...
#graph
  ...
  hc_caption(
    text = paste0('<a href=https://data.census.gov/cedsci/">Data Source: American Community Survey, 2015-2019 5-Year Estimate.</a>'),
     align = "left",
    style=list(fontSize='12px')
  ) %>% 
...
leaflet(height=585,width=785)%>%
  ...
  addLegend("bottomright", pal=pal, values = redist$quintil,
            title = "Redistricting Equity Index")
...
Black drivers (in LA City) are 5 times more likely to be stopped and nearly 9 times more likely to be arrested for traffic violations than White individuals, regardless of fault. 
...
We spend far too many of our tax dollars creating "Million Dollar Hoods" incarcerating people, when it is cheaper and healthier to offer people services. We spent $38 million incarcerating people from Westside South Central (an LAPD geography, and predominantly Black part of LA) from 2012 and 2017.
...
For First 5 LA to help ensure all children are ready for Kindergarten and for it's desired Results for Children and Families to happen, it will require sustained partnership with Black-led/serving organizations and communities to change the systems producing these outcomes. Here are immediate steps First 5 LA can take:

Follow LADPH action plan. According to an [LADPH action plan](http://publichealth.lacounty.gov/centerforhealthequity/PDF/AAIM-ActionPlan.pdf), reducing inequalities in infant mortality means reducing racism in systems, reducing poverty caused by housing and economic systems, reducing exposure to violence, as well as well individual actions.

Develop new forms of representation and power. Redistricting offers an opportunity to build community power through government representation. The last decade has seen many Black families leave South LA for the Antelope Valley and Inland Empire. Redistricting with developing Black representation and power in mind can help Black families have better access to resources and policies that serve them.

Build power with community-based organizations. Community-based organizations that are organizing constituencies for advocacy are a necessary component to bending public systems to the will of community members. Without a strong community advocacy voice, concerns of Black residents can be sidelined by powerful business or interests that can exert power on elected officials.

[GATHER FROM FIRST 5 LA]

# First 5 LA Goals and Provocative Questions

Pullout of Black data for indicators, results, and overall kindergarten readiness.
...
'Children have high-quality ECE experiences.')
 ...
 table%>%
   kbl(col.names=c("North Star or Result", "Status without systems change for Black Families"))%>%
