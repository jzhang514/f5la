...
Metro LA has the highest number of sheltered and unsheltered homeless individuals at nearly 3,000, despite being one of the smallest Best Start geographies. This figure includes individuals who are in shelters, transitional housing, or living outside the system in a vehicle or outside.<br>

```{r, include=FALSE}
###########################LAHSA 2019 RESULTS###########################################

#get data
housing<-dbGetQuery(con, "SELECT * FROM bsc_homeless_2019_new")

#remove la row from table for graphing because it skews the graph too much (LA homelessness = 51,000+)
housing<-subset(housing, best_start!="LA County")
...
housing$best_start <- factor(housing$best_start,levels = c("East LA", "Metro LA", "South El Monte/El Monte", "Southeast LA", "Broadway/Manchester", "Compton/East Compton", "Watts/Willowbrook", "West Athens", "North East Valley", "Panorama City & Neighbors", "Central Long Beach", "Wilmington", "Lancaster", "Palmdale"))

#graph
ggplot(housing, aes(x=best_start, y=totpeople))+
...
hc<-hchart(housing20, "column",
         hcaes(x=best_start, y=totpeople),
          tooltip = list(pointFormat = "Total: {point.totpeople}"))%>%
...
Region 1 has the largest population of sheltered and unsheltered homeless individuals at nearly 6,000. For all five regions, the overwhelming majority of homeless individuals are unsheltered, meaning that they are not connected to any safety net system.<br>
...
lahsa<-dbGetQuery(con, "SELECT * FROM lahsa_homeless_tract_2019")
  
lahsa<-lahsa[c(1:3, 4, 63:67)]

#join the lahsa data to best start
lahsa<-left_join(bsc_ct, lahsa, by=c("geoid"="ct_geoid"))

#join lahsa to region data
lahsa<-left_join(regions, lahsa, by=c("BSC" = "best_start"))

#subset to columns of interest
lahsa<-lahsa[c(2,18:22)]
...
lahsa<-rbind(lahsa, lb)


#create long data for graphing
lahsa<-pivot_longer(lahsa, c(totunsheltpeople:totsheltpeople), values_to = "value", names_to = "measure")

#graph

ggplot(lahsa, aes(fill=measure, y=value, x=Region))+
