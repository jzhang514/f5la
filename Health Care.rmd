                         ...
                         ifelse(pct_measure %in% 'pct_black_uninsured', 'Percent Black',
                                ifelse(pct_measure %in% 'pct_aian_uninsured', 'Percent AIAN',
                                       ifelse(pct_measure %in% 'pct_asian_uninsured', 'Percent Asian',
                                         ifelse(pct_measure %in% 'pct_nhpi_uninsured', 'Percent Native Hawaiian/Pacific Islander',
...
# broadwaymanchester<-insure_long_notot%>%
#   filter(grepl("Broadway/Manchester", best_start))
# 
# 
# highchart() %>%
#   hc_chart(type = "column", polar = TRUE) %>% 
#   hc_xAxis(categories = broadwaymanchester$variable) %>% 
#   hc_series(list(
#      name = "Percent",
#      data = broadwaymanchester$pct_value,
#      colorByPoint = TRUE,
#      type = "column",
#        colors=cols7,
#      showInLegend = FALSE
...
hchart(prenatal, "column",
    ...
    hc_yAxis(title = list(text = ""), 
      ...
      formatter = JS("function () {
       ...
       plotLines = list(
      list(
        label = list(text = "LA County = 8.2%", align = "left", y=-10, x=850),
        color = "#FF0000",
        width = 2,
        value = 8.2,
        zIndex=5
